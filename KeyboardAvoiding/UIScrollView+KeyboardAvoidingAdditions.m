...
- (BOOL)KeyboardAvoiding_focusNextTextField {
    ...
    UIView *view = [self KeyboardAvoiding_findNextInputViewAfterView:firstResponder beneathView:self];
...
- (UIView*)KeyboardAvoiding_findNextInputViewAfterView:(UIView*)priorView beneathView:(UIView*)view {
    UIView * candidate = nil;
    [self KeyboardAvoiding_findNextInputViewAfterView:priorView beneathView:view bestCandidate:&candidate];
    return candidate;
}

- (void)KeyboardAvoiding_findNextInputViewAfterView:(UIView*)priorView beneathView:(UIView*)view bestCandidate:(UIView**)bestCandidate {
    ...
    CGFloat bestCandidateHeuristic = [self KeyboardAvoiding_nextInputViewHeuristicForViewFrame:candidateFrame];
    
    for ( UIView *childView in view.subviews ) {
        if ( [self KeyboardAvoiding_viewIsValidKeyViewCandidate:childView] ) {
            CGRect frame = [self convertRect:childView.frame fromView:view];
            
            // Use a heuristic to evaluate candidates
            CGFloat heuristic = [self KeyboardAvoiding_nextInputViewHeuristicForViewFrame:frame];
            ...
            [self KeyboardAvoiding_findNextInputViewAfterView:priorView beneathView:childView bestCandidate:bestCandidate];
        }
    }
}

- (CGFloat)KeyboardAvoiding_nextInputViewHeuristicForViewFrame:(CGRect)frame {
    return  (-frame.origin.y * 1000.0) // Prefer elements closest to top (most important)
    + (-frame.origin.x);         // Prefer elements closest to left
}

- (BOOL)KeyboardAvoiding_viewHiddenOrUserInteractionNotEnabled:(UIView *)view {
    while ( view ) {
...
- (BOOL)KeyboardAvoiding_viewIsValidKeyViewCandidate:(UIView *)view {
    if ( [self KeyboardAvoiding_viewHiddenOrUserInteractionNotEnabled:view] ) return NO;
...
- (void)KeyboardAvoiding_initializeView:(UIView*)view {
    if ( [view isKindOfClass:[UITextField class]]
        ...
        UIView *otherView = [self KeyboardAvoiding_findNextInputViewAfterView:view beneathView:self];
